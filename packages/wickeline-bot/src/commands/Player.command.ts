...
const ReversedCharactters = Object.fromEntries(
  Object.entries(Characters).map(([k, v]) => [v, k])
);
const ReversedModes = Object.fromEntries(
  Object.entries(GameModes).map(([k, v]) => [v, k])
);
export class PlayerCommand extends Command {
  ...
  public async Run(message: Message, args) {
    ...
    const modeData = values.find(
      (record) => record.matchingTeamMode === localMode
    );

    if (!modeData) {
      return message.reply(
        `[Update] It looks like that test subject has hidden from ${ReversedModes[localMode]} in season ${season}`
      );
    }
