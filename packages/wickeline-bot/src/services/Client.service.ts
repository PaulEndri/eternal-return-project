...
export const ClientService = {
  ...
  getStats: async (
    ...
    if (
      ...
      if (
        new Date().valueOf() - record.lastUpdated.valueOf() <
        ClientService._cacheTime
      ) {
        return record.info;
      }
    }
  ...
  getMatches: async (id: number) => {
    ...
    if (
      new Date().valueOf() - cachedData.lastUpdated.valueOf() <
      ClientService._cacheTime
    ) {
      return cachedData.matches;
    }

    const limited = ClientService._limiter.wrap(
      ClientService._client.getGamesForPlayer
    );
    const results = await limited(id);

    results.forEach((match) => {
      if (
        cachedData.matches.some(
          (cachedMatch) => cachedMatch.gameId === match.gameId
        )
      ) {
        return;
      }
