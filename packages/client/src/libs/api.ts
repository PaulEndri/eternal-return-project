...
export class ErBsClient {
	...
	public async getCharacterAttributes() {
		return (await this.getMetaData(MetaTypes.CharacterAttributes)) as ICharacterAttribute[];
	}

	public async getCharacterLevelUpStats() {
		return (await this.getMetaData(MetaTypes.CharacterLevelUpStat)) as ICharacterLevelUpStat[];
	}
	 ...
	 * @param mode Solos/Duos/Squads
	 */
	public async getRankForPlayer(userId: number, season: number, mode: GameModes) {
