...
export class GenericApi {
  ...
  private getItems = async (route: string, full = false, force = false) => {
    ...
    for (const item of results) {
      ...
      const buildsInto = results
        .filter(
          ({ Material1, Material2 }) => Material1 === Name || Material2 === Name
        )
        .map(({ Name }) => Name);
  ...
  public getItemByName = async (name: string, full = false) => {
    return await this.getItems(
      `${GenericApi.BASE_URL}${GenericApi.ROUTES.ALL_ITEMS}`.replace(
        '/all',
        `?name=${name}`
      ),
      full,
